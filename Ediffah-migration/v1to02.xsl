...
<xsl:transform
  ...
  <xsl:output
    ...
    doctype-system="http://lcweb2.loc.gov/xmlcommon/dtds/ead2002/ead.dtd"/>
...
-->
  <xsl:param name='countrycode'>SE</xsl:param>
  <!-- Added to the <eadid> as @countrycode. Use ISO 3166-1 values.
-->
  <xsl:param name='mainagencycode'>KB</xsl:param>
  <!-- Added to <eadid> as @mainagencycode. Use ISO 15511 values.
     ...
     the repository code.
-->
  <xsl:param name='convdate'>2015-10-22</xsl:param>
  <!-- conversion date
...
-->
  <xsl:param name='isoconvdate'>20151022</xsl:param>
...
-->
  <xsl:param name='dtdpath'>http://lcweb2.loc.gov/xmlcommon/dtds/ead2002/ead.dtd</xsl:param>
...
-->
  <xsl:param name='converter'>v1to02.xsl (151022)</xsl:param>
                  ...
                  <xsl:for-each select='change'>
                    ...
                    <xsl:element name='change'>
                      ...
                      <xsl:element name='item'>
                        <xsl:value-of select='//eadid'/>
                        <xsl:text> konverterad från Ediffah (EAD 1.0) till EAD 2002 med </xsl:text>
                        <xsl:value-of select='$converter'/>
                        <xsl:text>.</xsl:text>
                      </xsl:element>
                    </xsl:element>
...
as both are not logically possible. -->
  ...
  <xsl:template match='did'>
    ...
    <xsl:copy>
      ...
      <!-- PKZ: Lägg till ediffah-id  -->
      <xsl:if test="//eadid/@identifier and (count(ancestor::*)=2)">
        <xsl:element name="unitid">
          <xsl:attribute name="type">
            <xsl:text>alternative</xsl:text>
          </xsl:attribute>
          <xsl:attribute name="label">
            <xsl:text>EDIFFAH</xsl:text>
          </xsl:attribute>
          <xsl:value-of select="//eadid/@identifier" />
        </xsl:element>
      </xsl:if>
